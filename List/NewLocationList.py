...
class RequirementEdge:
    def __init__(self,req=None,strict=True):
        if req is None:
...
class Locations:
    ...
    def __init__(self):
        self.lod_graph = self.makeLoDGraph()
        self.ag_graph = self.makeAGGraph()
        self.dc_graph = self.makeDCGraph()
        self.hundred_graph = self.makeHundredAcreGraph()
        self.oc_graph = self.makeOCGraph()
        self.bc_graph = self.makeBCGraph()
        self.sp_graph = self.makeSPGraph()
        self.ht_graph = self.makeHTGraph()
        self.pr_graph = self.makePRGraph()
        self.hb_graph = self.makeHBGraph()
        self.cor_graph = self.makeCoRGraph()
        self.pl_graph = self.makePLGraph()
        self.stt_graph = self.makeSTTGraph()
        self.tt_graph = self.makeTTGraph()
        self.twtnw_graph = self.makeTWTNWGraph()

    def getLoDGraph(self):
        return self.lod_graph

    def getAGGraph(self):
        return self.ag_graph

    def getDCGraph(self):
        return self.dc_graph

    def get100AcreGraph(self):
        return self.hundred_graph

    def getOCGraph(self):
        return self.oc_graph

    def getBCGraph(self):
        return self.bc_graph

    def getSPGraph(self):
        return self.sp_graph

    def getHTGraph(self):
        return self.ht_graph

    def getPRGraph(self):
        return self.pr_graph

    def getHBGraph(self):
        return self.hb_graph
    
    def getCoRGraph(self):
        return self.cor_graph
    
    def getPLGraph(self):
        return self.pl_graph

    def getSTTGraph(self):
        return self.stt_graph

    def getTTGraph(self):
        return self.tt_graph

    def getTWTNWGraph(self):
        return self.twtnw_graph


    def connectGraphs(self):
        g = self.getLoDGraph()
        g.add_edge("LoD-1","LoD-2",RequirementEdge())
        g.add_edge("LoD-2","LoD-3",RequirementEdge())
        g.add_edge("LoD-3","LoD-4",RequirementEdge())
        g.add_edge("LoD-4","LoD-5",RequirementEdge())
        g.add_edge("LoD-5","LoD-6",RequirementEdge())
        g.add_edge("LoD-6","LoD-7",RequirementEdge())
        g.add_edge("LoD-7","LoD-8",RequirementEdge())
        g.add_edge("LoD-8","LoD-9",RequirementEdge())
        g.add_edge("LoD-9","LoD-10",RequirementEdge())
        g.add_edge("LoD-10","LoD-11",RequirementEdge())
        g.add_edge("LoD-11","LoD-12",RequirementEdge())
    ...
    def getAllSoraLocations(self):
        return self.LoD() + self.AG() + self.DC() + self.HundredAcre() + \
                self.OC() + self.BC() + self.SP() + self.HT() + self.PR() + \
                self.HB() + self.CoR() + self.PL() + self.STT() + self.TT() + self.TWTNW() + \
                self.GoA() + self.Atl() + self.Levels() + self.FormLevels() + \
                self.StartingItems() + self.Puzzles() + self.WeaponList()
    ...
    def LoD(self):
        return self.create_list_from_nodes(self.lod_graph)

    def makeLoDGraph(self):
        g = Graph()
        g.add_node("LoD-1",LocationNode([KH2Location(245, "Bamboo Grove Dark Shard", locationCategory.CHEST,[locationType.LoD]),
        ...
        g.add_node("LoD-2",LocationNode([KH2Location(350, "Bamboo Grove Encampment Area Map", locationCategory.POPUP,[locationType.LoD]),]))
        g.add_node("LoD-3",LocationNode([KH2Location(417, "Mission 3", locationCategory.POPUP,[locationType.LoD]),]))
        g.add_node("LoD-4",LocationNode([KH2Location(21, "Checkpoint Hi-Potion", locationCategory.CHEST,[locationType.LoD]),
                                         KH2Location(121, "Checkpoint Mythril Shard", locationCategory.CHEST,[locationType.LoD]),]))
        g.add_node("LoD-5",LocationNode([KH2Location(22, "Mountain Trail Lightning Shard", locationCategory.CHEST,[locationType.LoD]),
        ...
        g.add_node("LoD-6",LocationNode([KH2Location(495, "Village Cave Area Map", locationCategory.POPUP,[locationType.LoD]),]))
        g.add_node("LoD-7",LocationNode([KH2Location(124, "Village Cave AP Boost", locationCategory.CHEST,[locationType.LoD]),
        ...
        g.add_node("LoD-8",LocationNode([KH2Location(24, "Ridge Frost Shard", locationCategory.CHEST,[locationType.LoD]),
                                         KH2Location(126, "Ridge AP Boost", locationCategory.CHEST,[locationType.LoD]),]))
        g.add_node("LoD-9",LocationNode([KH2Location(9, "Shan-Yu", locationCategory.HYBRIDBONUS,[locationType.LoD]),
                                         KH2Location(257, "Hidden Dragon", locationCategory.POPUP,[locationType.LoD]),]))
        g.add_node("LoD-10",LocationNode([KH2Location(25, "Throne Room Torn Pages", locationCategory.CHEST,[locationType.LoD]),
        ...
        g.add_node("LoD-11",LocationNode([KH2Location(10, "Storm Rider", locationCategory.ITEMBONUS,[locationType.LoD]),]))
        g.add_node("LoD-12",LocationNode([KH2Location(555, "Xigbar (Data) Defense Boost", locationCategory.POPUP,[locationType.LoD, locationType.DataOrg]),]))
        return g

    def AG(self):
        return self.create_list_from_nodes(self.ag_graph)

    def makeAGGraph(self):
        g = Graph()
        g.add_node("AG-1",LocationNode([KH2Location(353, "Agrabah Map", locationCategory.POPUP,[locationType.Agrabah]),]))
        g.add_node("AG-2",LocationNode([KH2Location(28, "Agrabah Dark Shard", locationCategory.CHEST,[locationType.Agrabah]),
        ...
        g.add_node("AG-3",LocationNode([KH2Location(31, "Bazaar Mythril Gem", locationCategory.CHEST,[locationType.Agrabah]),
        ...
        g.add_node("AG-4",LocationNode([KH2Location(136, "Palace Walls Skill Ring", locationCategory.CHEST,[locationType.Agrabah]),
                                        KH2Location(520, "Palace Walls Mythril Stone", locationCategory.CHEST,[locationType.Agrabah]),]))
        g.add_node("AG-5",LocationNode([KH2Location(250, "Cave Entrance Power Stone", locationCategory.CHEST,[locationType.Agrabah]),
                                        KH2Location(251, "Cave Entrance Mythril Shard", locationCategory.CHEST,[locationType.Agrabah]),]))
        g.add_node("AG-6",LocationNode([KH2Location(35, "Valley of Stone Mythril Stone", locationCategory.CHEST,[locationType.Agrabah]),
        ...
        g.add_node("AG-7",LocationNode([KH2Location(42, "Abu Escort", locationCategory.ITEMBONUS,[locationType.Agrabah]),]))
        g.add_node("AG-8",LocationNode([KH2Location(487, "Chasm of Challenges Cave of Wonders Map", locationCategory.CHEST,[locationType.Agrabah]),
                                        KH2Location(37, "Chasm of Challenges AP Boost", locationCategory.CHEST,[locationType.Agrabah]),]))
        g.add_node("AG-9",LocationNode([KH2Location(46, "Treasure Room", locationCategory.STATBONUS,[locationType.Agrabah]),]))
        g.add_node("AG-10",LocationNode([KH2Location(502, "Treasure Room AP Boost", locationCategory.CHEST,[locationType.Agrabah]),
                                         KH2Location(503, "Treasure Room Serenity Gem", locationCategory.CHEST,[locationType.Agrabah]),]))
        g.add_node("AG-11",LocationNode([KH2Location(37, "Elemental Lords", locationCategory.ITEMBONUS,[locationType.Agrabah]),
                                         KH2Location(300, "Lamp Charm", locationCategory.POPUP,[locationType.Agrabah]),]))
        g.add_node("AG-12",LocationNode([KH2Location(34, "Ruined Chamber Torn Pages", locationCategory.CHEST,[locationType.Agrabah]),
                                         KH2Location(486, "Ruined Chamber Ruins Map", locationCategory.CHEST,[locationType.Agrabah]),]))
        g.add_node("AG-13",LocationNode([KH2Location(15, "Genie Jafar", locationCategory.ITEMBONUS,[locationType.Agrabah]),
                                         KH2Location(303, "Wishing Lamp", locationCategory.POPUP,[locationType.Agrabah]),]))
        g.add_node("AG-14",LocationNode([KH2Location(65, "Lexaeus Bonus", locationCategory.STATBONUS,[locationType.Agrabah, locationType.AS]),
                                         KH2Location(545, "Lexaeus (AS) Strength Beyond Strength", locationCategory.POPUP,[locationType.Agrabah, locationType.AS]),]))
        g.add_node("AG-15",LocationNode([KH2Location(550, "Lexaeus (Data) Lost Illusion", locationCategory.POPUP,[locationType.Agrabah, locationType.DataOrg]),]))
        return g

    def DC(self):
        return self.create_list_from_nodes(self.dc_graph)

    def makeDCGraph(self):
        g = Graph()

        g.add_node("DC-1",LocationNode([KH2Location(16, "DC Courtyard Mythril Shard", locationCategory.CHEST,[locationType.DC]),
        ...
        g.add_node("DC-2",LocationNode([KH2Location(91, "Library Torn Pages", locationCategory.CHEST,[locationType.DC]),]))
        g.add_node("DC-3",LocationNode([KH2Location(332, "Disney Castle Map", locationCategory.POPUP,[locationType.DC]),]))
        g.add_node("DC-4",LocationNode([KH2Location(38, "Minnie Escort", locationCategory.HYBRIDBONUS,[locationType.DC]),]))
        g.add_node("DC-5",LocationNode([KH2Location(79, "Cornerstone Hill Map", locationCategory.CHEST,[locationType.DC]),
                                        KH2Location(12, "Cornerstone Hill Frost Shard", locationCategory.CHEST,[locationType.DC]),]))
        g.add_node("DC-6",LocationNode([KH2Location(81, "Pier Mythril Shard", locationCategory.CHEST,[locationType.DC]),
                                        KH2Location(82, "Pier Hi-Potion", locationCategory.CHEST,[locationType.DC]),]))
        g.add_node("DC-7",LocationNode([KH2Location(83, "Waterway Mythril Stone", locationCategory.CHEST,[locationType.DC]),
        ...
        g.add_node("DC-8",LocationNode([KH2Location(368, "Window of Time Map", locationCategory.POPUP,[locationType.DC]),]))
        g.add_node("DC-9",LocationNode([KH2Location(16, "Boat Pete", locationCategory.ITEMBONUS,[locationType.DC]),]))
        g.add_node("DC-10",LocationNode([KH2Location(17, "Future Pete", locationCategory.HYBRIDBONUS,[locationType.DC]),
                                        KH2Location(261, "Monochrome", locationCategory.POPUP,[locationType.DC]),]))
        g.add_node("DC-11",LocationNode([KH2Location(262, "Wisdom Form", locationCategory.POPUP,[locationType.DC]),]))
        g.add_node("DC-12",LocationNode([KH2Location(67, "Marluxia Bonus", locationCategory.STATBONUS,[locationType.DC, locationType.AS]),
                                        KH2Location(548, "Marluxia (AS) Eternal Blossom", locationCategory.POPUP,[locationType.DC, locationType.AS]),]))
        g.add_node("DC-13",LocationNode([KH2Location(553, "Marluxia (Data) Lost Illusion", locationCategory.POPUP,[locationType.DC, locationType.DataOrg]),]))
        g.add_node("DC-14",LocationNode([KH2Location(70, "Lingering Will Bonus", locationCategory.STATBONUS,[locationType.HT, locationType.LW], InvalidChecks=[itemType.PROOF_OF_CONNECTION]),
    ...
    def HundredAcre(self):
        return self.create_list_from_nodes(self.hundred_graph)

    def makeHundredAcreGraph(self):
        g = Graph()

        g.add_node("100-1",LocationNode([KH2Location(313, "Pooh's House 100 Acre Wood Map", locationCategory.CHEST,[locationType.HUNDREDAW]),
        ...
        g.add_node("100-2",LocationNode([KH2Location(105, "Piglet's House Defense Boost", locationCategory.CHEST,[locationType.HUNDREDAW]),
        ...
        g.add_node("100-3",LocationNode([KH2Location(314, "Rabbit's House Draw Ring", locationCategory.CHEST,[locationType.HUNDREDAW]),
        ...
        g.add_node("100-4",LocationNode([KH2Location(108, "Kanga's House Magic Boost", locationCategory.CHEST,[locationType.HUNDREDAW]),
        ...
        g.add_node("100-5",LocationNode([KH2Location(110, "Spooky Cave Mythril Gem", locationCategory.CHEST,[locationType.HUNDREDAW]),
        ...
        g.add_node("100-6",LocationNode([KH2Location(312, "Starry Hill Cosmic Ring", locationCategory.CHEST,[locationType.HUNDREDAW]),
    ...
    def OC(self):
        return self.create_list_from_nodes(self.oc_graph)

    def makeOCGraph(self):
        g = Graph()

        g.add_node("OC-1",LocationNode([KH2Location(7,"Passage Mythril Shard", locationCategory.CHEST,[locationType.OC]),
        ...
        g.add_node("OC-2",LocationNode([KH2Location(2,"Inner Chamber Underworld Map", locationCategory.CHEST,[locationType.OC]),
                                        KH2Location(243, "Inner Chamber Mythril Shard", locationCategory.CHEST,[locationType.OC]),]))
        g.add_node("OC-3",LocationNode([KH2Location(5, "Cerberus", locationCategory.ITEMBONUS,[locationType.OC]),]))
        g.add_node("OC-4",LocationNode([KH2Location(338, "Coliseum Map", locationCategory.POPUP,[locationType.OC]),]))
        g.add_node("OC-5",LocationNode([KH2Location(57, "Urns", locationCategory.ITEMBONUS,[locationType.OC]),]))
        g.add_node("OC-6",LocationNode([KH2Location(242, "Underworld Entrance Power Boost", locationCategory.CHEST,[locationType.OC]),]))
        g.add_node("OC-7",LocationNode([KH2Location(3,"Caverns Entrance Lucid Shard", locationCategory.CHEST,[locationType.OC]),
        ...
        g.add_node("OC-8",LocationNode([KH2Location(9,"The Lost Road Bright Shard", locationCategory.CHEST,[locationType.OC]),
        ...
        g.add_node("OC-9",LocationNode([KH2Location(150, "Atrium Lucid Stone", locationCategory.CHEST,[locationType.OC]),
                                        KH2Location(151, "Atrium AP Boost", locationCategory.CHEST,[locationType.OC]),]))
        g.add_node("OC-10",LocationNode([KH2Location(58, "Demyx OC", locationCategory.STATBONUS,[locationType.OC]),
        ...
        g.add_node("OC-11",LocationNode([KH2Location(244, "The Lock Caverns Map", locationCategory.CHEST,[locationType.OC]),
        ...
        g.add_node("OC-12",LocationNode([KH2Location(6, "Pete (OC)", locationCategory.ITEMBONUS,[locationType.OC]),]))
        g.add_node("OC-13",LocationNode([KH2Location(7, "Hydra", locationCategory.HYBRIDBONUS,[locationType.OC]),
                                        KH2Location(260, "Hero´s Crest", locationCategory.POPUP,[locationType.OC]),]))
        g.add_node("OC-14",LocationNode([KH2Location(295, "Auron´s Statue", locationCategory.POPUP,[locationType.OC]),]))
        g.add_node("OC-15",LocationNode([KH2Location(8, "Hades", locationCategory.HYBRIDBONUS,[locationType.OC]),
                                        KH2Location(272, "Guardian Soul", locationCategory.POPUP,[locationType.OC]),]))
        g.add_node("OC-16",LocationNode([KH2Location(513, "Protect Belt (Pain and Panic Cup)", locationCategory.POPUP,[locationType.OC, locationType.OCCups], InvalidChecks=[itemType.TROPHY]),
        ...
        g.add_node("OC-17",LocationNode([KH2Location(515, "Rising Dragon (Cerberus Cup)", locationCategory.POPUP,[locationType.OC, locationType.OCCups], InvalidChecks=[itemType.TROPHY]),
        ...
        g.add_node("OC-18",LocationNode([KH2Location(514, "Genji Shield (Titan Cup)", locationCategory.POPUP,[locationType.OC, locationType.OCCups], InvalidChecks=[itemType.TROPHY]),
        ...
        g.add_node("OC-19",LocationNode([KH2Location(516, "Fatal Crest (Goddess of Fate Cup)", locationCategory.POPUP,[locationType.OC, locationType.OCCups], InvalidChecks=[itemType.TROPHY]),
        ...
        g.add_node("OC-20",LocationNode([KH2Location(518, "Hades Cup Trophy (Paradox Cups)", locationCategory.POPUP,[locationType.OC, locationType.OCCups, locationType.OCParadoxCup], InvalidChecks=[itemType.TROPHY, itemType.FORM, itemType.SUMMON]),]))
        g.add_node("OC-21",LocationNode([KH2Location(66, "Zexion Bonus", locationCategory.STATBONUS,[locationType.OC, locationType.AS]),
                                        KH2Location(546, "Zexion (AS) Book of Shadows", locationCategory.POPUP,[locationType.OC, locationType.AS]),]))
        g.add_node("OC-22",LocationNode([KH2Location(551, "Zexion (Data) Lost Illusion", locationCategory.POPUP,[locationType.OC, locationType.DataOrg]),]))

        return g

    def BC(self):
        return self.create_list_from_nodes(self.bc_graph)

    def makeBCGraph(self):
        g = Graph()

        g.add_node("BC-1",LocationNode([KH2Location(39, "BC Courtyard AP Boost", locationCategory.CHEST,[locationType.BC]),
        ...
        g.add_node("BC-2",LocationNode([KH2Location(46, "Belle's Room Castle Map", locationCategory.CHEST,[locationType.BC]),
                                        KH2Location(240, "Belle's Room Mega-Recipe", locationCategory.CHEST,[locationType.BC]),]))
        g.add_node("BC-3",LocationNode([KH2Location(63, "The East Wing Mythril Shard", locationCategory.CHEST,[locationType.BC]),
                                        KH2Location(155, "The East Wing Tent", locationCategory.CHEST,[locationType.BC]),]))
        g.add_node("BC-4",LocationNode([KH2Location(41, "The West Hall Hi-Potion", locationCategory.CHEST,[locationType.BC]),
        ...
        g.add_node("BC-5",LocationNode([KH2Location(2, "Thresholder", locationCategory.ITEMBONUS,[locationType.BC]),]))
        g.add_node("BC-6",LocationNode([KH2Location(239, "Dungeon Basement Map", locationCategory.CHEST,[locationType.BC]),
                                        KH2Location(43, "Dungeon AP Boost", locationCategory.CHEST,[locationType.BC]),]))
        g.add_node("BC-7",LocationNode([KH2Location(44, "Secret Passage Mythril Shard", locationCategory.CHEST,[locationType.BC]),
        ...
        g.add_node("BC-8",LocationNode([KH2Location(208, "The West Hall Mythril Shard (Post Dungeon)", locationCategory.CHEST,[locationType.BC]),]))
        g.add_node("BC-9",LocationNode([KH2Location(42, "The West Wing Mythril Shard", locationCategory.CHEST,[locationType.BC]),
                                        KH2Location(164, "The West Wing Tent", locationCategory.CHEST,[locationType.BC]),]))
        g.add_node("BC-10",LocationNode([KH2Location(12, "Beast", locationCategory.STATBONUS,[locationType.BC]),]))
        g.add_node("BC-11",LocationNode([KH2Location(241, "The Beast's Room Blazing Shard", locationCategory.CHEST,[locationType.BC]),]))
        g.add_node("BC-12",LocationNode([KH2Location(3, "Dark Thorn Bonus", locationCategory.HYBRIDBONUS,[locationType.BC]),
                                        KH2Location(299, "Dark Thorn Cure Element", locationCategory.POPUP,[locationType.BC]),]))
        g.add_node("BC-13",LocationNode([KH2Location(270, "Rumbling Rose", locationCategory.POPUP,[locationType.BC]),
                                        KH2Location(325, "Castle Walls Map", locationCategory.POPUP,[locationType.BC]),]))
        g.add_node("BC-14",LocationNode([KH2Location(4, "Xaldin Bonus", locationCategory.HYBRIDBONUS,[locationType.BC]),
                                        KH2Location(528, "Secret Ansem Report 6", locationCategory.POPUP,[locationType.BC]),]))
        g.add_node("BC-15",LocationNode([KH2Location(559, "Xaldin (Data) Defense Boost", locationCategory.POPUP,[locationType.BC, locationType.DataOrg]),]))

        return g

    def SP(self):
        return self.create_list_from_nodes(self.sp_graph)

    def makeSPGraph(self):
        g = Graph()

        g.add_node("SP-1",LocationNode([KH2Location(316, "Pit Cell Area Map", locationCategory.CHEST,[locationType.SP]),
                                        KH2Location(64, "Pit Cell Mythril Crystal", locationCategory.CHEST,[locationType.SP]),]))
        g.add_node("SP-2",LocationNode([KH2Location(65, "Canyon Dark Crystal", locationCategory.CHEST,[locationType.SP]),
        ...
        g.add_node("SP-3",LocationNode([KH2Location(45, "Screens", locationCategory.STATBONUS,[locationType.SP]),]))
        g.add_node("SP-4",LocationNode([KH2Location(49, "Hallway Power Crystal", locationCategory.CHEST,[locationType.SP]),
                                        KH2Location(50, "Hallway AP Boost", locationCategory.CHEST,[locationType.SP]),]))
        g.add_node("SP-5",LocationNode([KH2Location(255, "Communications Room I/O Tower Map", locationCategory.CHEST,[locationType.SP]),
                                        KH2Location(499, "Communications Room Gaia Belt", locationCategory.CHEST,[locationType.SP]),]))
        g.add_node("SP-6",LocationNode([KH2Location(31, "Hostile Program", locationCategory.HYBRIDBONUS,[locationType.SP]),]))
        g.add_node("SP-7",LocationNode([KH2Location(267, "Photon Debugger", locationCategory.POPUP,[locationType.SP]),]))
        g.add_node("SP-8",LocationNode([KH2Location(61, "Solar Sailer", locationCategory.ITEMBONUS,[locationType.SP]),]))
        g.add_node("SP-9",LocationNode([KH2Location(177, "Central Computer Core AP Boost", locationCategory.CHEST,[locationType.SP]),
        ...
        g.add_node("SP-10",LocationNode([KH2Location(32, "MCP", locationCategory.HYBRIDBONUS,[locationType.SP]),]))
        g.add_node("SP-11",LocationNode([KH2Location(68, "Larxene Bonus", locationCategory.STATBONUS,[locationType.SP, locationType.AS]),
                                        KH2Location(547, "Larxene (AS) Cloaked Thunder", locationCategory.POPUP,[locationType.SP, locationType.AS]),]))
        g.add_node("SP-12",LocationNode([KH2Location(552, "Larxene (Data) Lost Illusion", locationCategory.POPUP,[locationType.SP, locationType.DataOrg]),]))

        return g

    def HT(self):
        return self.create_list_from_nodes(self.ht_graph)

    def makeHTGraph(self):
        g = Graph()

        g.add_node("HT-1",LocationNode([KH2Location(53, "Graveyard Mythril Shard", locationCategory.CHEST,[locationType.HT]),
                                        KH2Location(212, "Graveyard Serenity Gem", locationCategory.CHEST,[locationType.HT]),]))
        g.add_node("HT-2",LocationNode([KH2Location(211, "Finklestein's Lab Halloween Town Map", locationCategory.CHEST,[locationType.HT]),]))
        g.add_node("HT-3",LocationNode([KH2Location(209, "Town Square Mythril Stone", locationCategory.CHEST,[locationType.HT]),
                                        KH2Location(210, "Town Square Energy Shard", locationCategory.CHEST,[locationType.HT]),]))
        g.add_node("HT-4",LocationNode([KH2Location(54, "Hinterlands Lightning Shard", locationCategory.CHEST,[locationType.HT]),
        ...
        g.add_node("HT-5",LocationNode([KH2Location(55, "Candy Cane Lane Mega-Potion", locationCategory.CHEST,[locationType.HT]),
        ...
        g.add_node("HT-6",LocationNode([KH2Location(57, "Santa's House Christmas Town Map", locationCategory.CHEST,[locationType.HT]),
                                        KH2Location(58, "Santa's House AP Boost", locationCategory.CHEST,[locationType.HT]),]))
        g.add_node("HT-7",LocationNode([KH2Location(18, "Prison Keeper", locationCategory.HYBRIDBONUS,[locationType.HT]),]))
        g.add_node("HT-8",LocationNode([KH2Location(19, "Oogie Boogie", locationCategory.STATBONUS,[locationType.HT]),
                                        KH2Location(301, "Oogie Boogie Magnet Element", locationCategory.POPUP,[locationType.HT]),]))
        g.add_node("HT-9",LocationNode([KH2Location(40, "Lock, Shock, and Barrel", locationCategory.STATBONUS,[locationType.HT]),]))
        g.add_node("HT-10",LocationNode([KH2Location(297, "Present", locationCategory.POPUP,[locationType.HT]),]))
        g.add_node("HT-11",LocationNode([KH2Location(298, "Decoy Presents", locationCategory.POPUP,[locationType.HT]),]))
        g.add_node("HT-12",LocationNode([KH2Location(20, "Experiment", locationCategory.STATBONUS,[locationType.HT]),
                                        KH2Location(275, "Decisive Pumpkin", locationCategory.POPUP,[locationType.HT]),]))
        g.add_node("HT-13",LocationNode([KH2Location(64, "Vexen Bonus", locationCategory.STATBONUS,[locationType.HT, locationType.AS]),
                                        KH2Location(544, "Vexen (AS) Road to Discovery", locationCategory.POPUP,[locationType.HT, locationType.AS]),]))
        g.add_node("HT-14",LocationNode([KH2Location(549, "Vexen (Data) Lost Illusion", locationCategory.POPUP,[locationType.HT, locationType.DataOrg])]))

        return g

    def PR(self):
        return self.create_list_from_nodes(self.pr_graph)
    
    def makePRGraph(self):
        g = Graph()

        g.add_node("PR-1",LocationNode([KH2Location(70, "Rampart Naval Map", locationCategory.CHEST,[locationType.PR]),
        ...
        g.add_node("PR-2",LocationNode([KH2Location(71, "Town Dark Stone", locationCategory.CHEST,[locationType.PR]),
        ...
        g.add_node("PR-3",LocationNode([KH2Location(74, "Cave Mouth Bright Shard", locationCategory.CHEST,[locationType.PR]),
                                        KH2Location(223, "Cave Mouth Mythril Shard", locationCategory.CHEST,[locationType.PR]),]))
        g.add_node("PR-4",LocationNode([KH2Location(329, "Isla de Muerta Map", locationCategory.POPUP,[locationType.PR]),]))
        g.add_node("PR-5",LocationNode([KH2Location(62, "Boat Fight", locationCategory.ITEMBONUS,[locationType.PR]),]))
        g.add_node("PR-6",LocationNode([KH2Location(39, "Interceptor Barrels", locationCategory.STATBONUS,[locationType.PR]),]))
        g.add_node("PR-7",LocationNode([KH2Location(369, "Powder Store AP Boost (1)", locationCategory.CHEST,[locationType.PR]),
                                        KH2Location(370, "Powder Store AP Boost (2)", locationCategory.CHEST,[locationType.PR]),]))
        g.add_node("PR-8",LocationNode([KH2Location(75, "Moonlight Nook Mythril Shard", locationCategory.CHEST,[locationType.PR]),
        ...
        g.add_node("PR-9",LocationNode([KH2Location(21, "Barbossa", locationCategory.HYBRIDBONUS,[locationType.PR]),
                                        KH2Location(263, "Follow the Wind", locationCategory.POPUP,[locationType.PR]),]))
        g.add_node("PR-10",LocationNode([KH2Location(59, "Grim Reaper 1", locationCategory.ITEMBONUS,[locationType.PR]),]))
        g.add_node("PR-11",LocationNode([KH2Location(252, "Interceptor's Hold Feather Charm", locationCategory.CHEST,[locationType.PR]),]))
        g.add_node("PR-12",LocationNode([KH2Location(76, "Seadrift Keep AP Boost", locationCategory.CHEST,[locationType.PR]),
        ...
        g.add_node("PR-13",LocationNode([KH2Location(77, "Seadrift Row Serenity Gem", locationCategory.CHEST,[locationType.PR]),
        ...
        g.add_node("PR-14",LocationNode([KH2Location(296, "Seadrift Row Cursed Medallion", locationCategory.POPUP,[locationType.PR]),
                                        KH2Location(331, "Seadrift Row Ship Graveyard Map", locationCategory.POPUP,[locationType.PR]),]))
        g.add_node("PR-15",LocationNode([KH2Location(22, "Grim Reaper 2", locationCategory.ITEMBONUS,[locationType.PR]),
                                        KH2Location(530, "Secret Ansem Report 4", locationCategory.POPUP,[locationType.PR]),]))
        g.add_node("PR-16",LocationNode([KH2Location(557, "Luxord (Data) AP Boost", locationCategory.POPUP,[locationType.PR, locationType.DataOrg]),]))

        return g

    def HB(self):
        return self.create_list_from_nodes(self.hb_graph)

    def makeHBGraph(self):
        g = Graph()

        g.add_node("HB-1",LocationNode([KH2Location(362, "Marketplace Map", locationCategory.POPUP,[locationType.HB]),]))
        g.add_node("HB-2",LocationNode([KH2Location(194, "Borough Drive Recovery", locationCategory.CHEST,[locationType.HB]),
        ...
        g.add_node("HB-3",LocationNode([KH2Location(256, "Merlin's House Membership Card", locationCategory.POPUP,[locationType.HB]),
                                        KH2Location(292, "Merlin's House Blizzard Element", locationCategory.POPUP,[locationType.HB]),]))
        g.add_node("HB-4",LocationNode([KH2Location(47, "Bailey", locationCategory.ITEMBONUS,[locationType.HB]),
                                        KH2Location(531, "Bailey Secret Ansem Report 7", locationCategory.POPUP,[locationType.HB]),]))
        g.add_node("HB-5",LocationNode([KH2Location(258, "Baseball Charm", locationCategory.POPUP,[locationType.HB]),]))
        g.add_node("HB-6",LocationNode([KH2Location(310, "Postern Castle Perimeter Map", locationCategory.CHEST,[locationType.HB]),
        ...
        g.add_node("HB-7",LocationNode([KH2Location(200, "Corridors Mythril Stone", locationCategory.CHEST,[locationType.HB]),
        ...
        g.add_node("HB-8",LocationNode([KH2Location(266, "Ansem's Study Master Form", locationCategory.POPUP,[locationType.HB]),
                                        KH2Location(276, "Ansem's Study Sleeping Lion", locationCategory.POPUP,[locationType.HB]),]))
        g.add_node("HB-9",LocationNode([KH2Location(184, "Ansem's Study Skill Recipe", locationCategory.CHEST,[locationType.HB]),]))
        g.add_node("HB-10",LocationNode([KH2Location(183, "Ansem's Study Ukulele Charm", locationCategory.CHEST,[locationType.HB]),]))
        g.add_node("HB-11",LocationNode([KH2Location(309, "Restoration Site Moon Recipe", locationCategory.CHEST,[locationType.HB]),
                                        KH2Location(507, "Restoration Site AP Boost", locationCategory.CHEST,[locationType.HB]),]))
        g.add_node("HB-12",LocationNode([KH2Location(28, "Demyx (HB)", locationCategory.HYBRIDBONUS,[locationType.HB]),]))
        g.add_node("HB-13",LocationNode([KH2Location(361, "FF Fights Cure Element", locationCategory.POPUP,[locationType.HB]),]))
        g.add_node("HB-14",LocationNode([KH2Location(179, "Crystal Fissure Torn Pages", locationCategory.CHEST,[locationType.HB]),
        ...
        g.add_node("HB-15",LocationNode([KH2Location(60, "1000 Heartless", locationCategory.ITEMBONUS,[locationType.HB]),
        ...
        g.add_node("HB-16",LocationNode([KH2Location(491, "Postern Gull Wing", locationCategory.CHEST,[locationType.HB]),]))
        g.add_node("HB-17",LocationNode([KH2Location(311, "Heartless Manufactory Cosmic Chain", locationCategory.CHEST,[locationType.HB], InvalidChecks=[itemType.MEMBERSHIPCARD]),]))
        g.add_node("HB-18",LocationNode([KH2Location(35, "Sephiroth Bonus", locationCategory.STATBONUS,[locationType.HB, locationType.Sephi]),
                                        KH2Location(282, "Sephiroth Fenrir", locationCategory.POPUP,[locationType.HB, locationType.Sephi]),]))
        g.add_node("HB-19",LocationNode([KH2Location(588, "Winner's Proof", locationCategory.POPUP,[locationType.HB, locationType.Mush13], InvalidChecks=[itemType.PROOF_OF_PEACE]),
        ...
        g.add_node("HB-20",LocationNode([KH2Location(560, "Demyx (Data) AP Boost", locationCategory.POPUP,[locationType.HB, locationType.DataOrg], InvalidChecks=[itemType.FORM]),]))

        return g

    def CoR(self):
        return self.create_list_from_nodes(self.cor_graph)

    def makeCoRGraph(self):
        g = Graph()

        g.add_node("CoR-1",LocationNode([KH2Location(562, "CoR Depths AP Boost", locationCategory.CHEST,[locationType.HB, locationType.CoR]),
        ...
        g.add_node("CoR-2",LocationNode([KH2Location(580, "CoR Mineshaft Lower Level Depths of Remembrance Map", locationCategory.CHEST,[locationType.HB, locationType.CoR]),
                                        KH2Location(578, "CoR Mineshaft Lower Level AP Boost", locationCategory.CHEST,[locationType.HB, locationType.CoR]),]))
        g.add_node("CoR-3",LocationNode([KH2Location(567, "CoR Depths Upper Level Remembrance Gem", locationCategory.CHEST,[locationType.HB, locationType.CoR]),]))
        g.add_node("CoR-4",LocationNode([KH2Location(568, "CoR Mining Area Serenity Gem", locationCategory.CHEST,[locationType.HB, locationType.CoR]),
        ...
        g.add_node("CoR-5",LocationNode([KH2Location(581, "CoR Mineshaft Mid Level Power Boost", locationCategory.CHEST,[locationType.HB, locationType.CoR]),]))
        g.add_node("CoR-6",LocationNode([KH2Location(574, "CoR Engine Chamber Serenity Crystal", locationCategory.CHEST,[locationType.HB, locationType.CoR]),
        ...
        g.add_node("CoR-7",LocationNode([KH2Location(582, "CoR Mineshaft Upper Level Magic Boost", locationCategory.CHEST,[locationType.HB, locationType.CoR]),]))
        g.add_node("CoR-8",LocationNode([KH2Location(579, "CoR Mineshaft Upper Level AP Boost", locationCategory.CHEST,[locationType.HB, locationType.CoR]),]))
        g.add_node("CoR-9",LocationNode([KH2Location(72, "Transport to Remembrance", locationCategory.STATBONUS,[locationType.HB, locationType.CoR, locationType.TTR]),]))

        return g

    def PL(self):
        return self.create_list_from_nodes(self.pl_graph)

    def makePLGraph(self):
        g = Graph()

        g.add_node("PL-1",LocationNode([KH2Location(492, "Gorge Savannah Map", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-2",LocationNode([KH2Location(401, "Elephant Graveyard Frost Gem", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-3",LocationNode([KH2Location(418, "Pride Rock Map", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-4",LocationNode([KH2Location(396, "Wildebeest Valley Energy Stone", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-5",LocationNode([KH2Location(406, "Wastelands Mythril Shard", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-6",LocationNode([KH2Location(409, "Jungle Serenity Gem", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-7",LocationNode([KH2Location(412, "Oasis Map", locationCategory.CHEST,[locationType.PL]),
        ...
        g.add_node("PL-8",LocationNode([KH2Location(264, "Circle of Life", locationCategory.POPUP,[locationType.PL]),]))
        g.add_node("PL-9",LocationNode([KH2Location(49, "Hyenas 1", locationCategory.STATBONUS,[locationType.PL]),]))
        g.add_node("PL-10",LocationNode([KH2Location(29, "Scar", locationCategory.STATBONUS,[locationType.PL]),
                                        KH2Location(302, "Scar Fire Element", locationCategory.POPUP,[locationType.PL]),]))
        g.add_node("PL-11",LocationNode([KH2Location(50, "Hyenas 2", locationCategory.STATBONUS,[locationType.PL]),]))
        g.add_node("PL-12",LocationNode([KH2Location(30, "Groundshaker", locationCategory.HYBRIDBONUS,[locationType.PL]),]))
        g.add_node("PL-13",LocationNode([KH2Location(556, "Saix (Data) Defense Boost", locationCategory.POPUP,[locationType.PL, locationType.DataOrg]),]))

        return g

    def STT(self):
        return self.create_list_from_nodes(self.stt_graph)

    def makeSTTGraph(self):
        g = Graph()

        g.add_node("STT-1",LocationNode([KH2Location(319, "Twilight Town Map", locationCategory.POPUP,[locationType.STT]),]))
        g.add_node("STT-2",LocationNode([KH2Location(288, "Munny Pouch (Olette)", locationCategory.POPUP,[locationType.STT]),]))
        g.add_node("STT-3",LocationNode([KH2Location(54, "Station Dusks", locationCategory.ITEMBONUS,[locationType.STT]),
        ...
        g.add_node("STT-4",LocationNode([KH2Location(33, "Twilight Thorn", locationCategory.ITEMBONUS,[locationType.STT]),]))
        g.add_node("STT-5",LocationNode([KH2Location(73, "Axel 1", locationCategory.ITEMBONUS,[locationType.STT]),]))
        g.add_node("STT-6",LocationNode([KH2Location(389, "(Junk) Champion Belt", locationCategory.CHEST,[locationType.STT], InvalidChecks=[e for e in itemType if e not in [itemType.SYNTH,itemType.ITEM] ]),
        ...
        g.add_node("STT-7",LocationNode([KH2Location(428, "Central Station Potion (1)", locationCategory.CHEST,[locationType.STT]),
        ...
        g.add_node("STT-8",LocationNode([KH2Location(434, "Sunset Terrace Ability Ring", locationCategory.CHEST,[locationType.STT]),
        ...
        g.add_node("STT-9",LocationNode([KH2Location(449, "Mansion Foyer Hi-Potion", locationCategory.CHEST,[locationType.STT]),
        ...
        g.add_node("STT-10",LocationNode([KH2Location(455, "Mansion Dining Room Elven Bandanna", locationCategory.CHEST,[locationType.STT]),
                                        KH2Location(456, "Mansion Dining Room Potion", locationCategory.CHEST,[locationType.STT]),]))
        g.add_node("STT-11",LocationNode([KH2Location(289, "Naminé´s Sketches", locationCategory.POPUP,[locationType.STT]),
                                        KH2Location(483, "Mansion Map", locationCategory.POPUP,[locationType.STT]),]))
        g.add_node("STT-12",LocationNode([KH2Location(459, "Mansion Library Hi-Potion", locationCategory.CHEST,[locationType.STT]),]))
        g.add_node("STT-13",LocationNode([KH2Location(34, "Axel 2", locationCategory.STATBONUS,[locationType.STT]),]))
        g.add_node("STT-14",LocationNode([KH2Location(463, "Mansion Basement Corridor Hi-Potion", locationCategory.CHEST,[locationType.STT]),]))
        g.add_node("STT-15",LocationNode([KH2Location(558, "Roxas (Data) Magic Boost", locationCategory.POPUP,[locationType.STT, locationType.DataOrg]),]))

        return g

    def TT(self):
        return self.create_list_from_nodes(self.tt_graph)
    
    def makeTTGraph(self):
        g = Graph()

        g.add_node("TT-1",LocationNode([KH2Location(447, "Old Mansion Potion", locationCategory.CHEST,[locationType.TT]),
                                        KH2Location(448, "Old Mansion Mythril Shard", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-2",LocationNode([KH2Location(442, "The Woods Potion", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-3",LocationNode([KH2Location(420, "Tram Common Hi-Potion", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-4",LocationNode([KH2Location(526, "Station Plaza Secret Ansem Report 2", locationCategory.POPUP,[locationType.TT]),
        ...
        g.add_node("TT-5",LocationNode([KH2Location(431, "Central Station Tent", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-6",LocationNode([KH2Location(465, "The Tower Potion", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-7",LocationNode([KH2Location(467, "Tower Entryway Ether", locationCategory.CHEST,[locationType.TT]),
                                        KH2Location(468, "Tower Entryway Mythril Shard", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-8",LocationNode([KH2Location(469, "Sorcerer's Loft Tower Map", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-9",LocationNode([KH2Location(470, "Tower Wardrobe Mythril Stone", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-10",LocationNode([KH2Location(304, "Star Seeker", locationCategory.POPUP,[locationType.TT]),
                                        KH2Location(286, "Valor Form", locationCategory.POPUP,[locationType.TT]),]))
        g.add_node("TT-11",LocationNode([KH2Location(294, "Seifer´s Trophy", locationCategory.POPUP,[locationType.TT]),]))
        g.add_node("TT-12",LocationNode([KH2Location(265, "Oathkeeper", locationCategory.POPUP,[locationType.TT]),
                                        KH2Location(543, "Limit Form", locationCategory.POPUP,[locationType.TT]),]))
        g.add_node("TT-13",LocationNode([KH2Location(479, "Underground Concourse Mythril Gem", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-14",LocationNode([KH2Location(477, "Tunnelway Orichalcum", locationCategory.CHEST,[locationType.TT]),
                                        KH2Location(478, "Tunnelway Mythril Crystal", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-15",LocationNode([KH2Location(438, "Sunset Terrace Orichalcum+", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-16",LocationNode([KH2Location(56, "Mansion Nobodies", locationCategory.STATBONUS,[locationType.TT]),]))
        g.add_node("TT-17",LocationNode([KH2Location(452, "Mansion Foyer Mythril Crystal", locationCategory.CHEST,[locationType.TT]),
        ...
        g.add_node("TT-18",LocationNode([KH2Location(457, "Mansion Dining Room Mythril Crystal", locationCategory.CHEST,[locationType.TT]),
                                        KH2Location(458, "Mansion Dining Room Mythril Stone", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-19",LocationNode([KH2Location(460, "Mansion Library Orichalcum", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-20",LocationNode([KH2Location(534, "Beam Secret Ansem Report 10", locationCategory.POPUP,[locationType.TT]),]))
        g.add_node("TT-21",LocationNode([KH2Location(464, "Mansion Basement Corridor Ultimate Recipe", locationCategory.CHEST,[locationType.TT]),]))
        g.add_node("TT-22",LocationNode([KH2Location(63, "Betwixt and Between", locationCategory.ITEMBONUS,[locationType.TT]),
                                        KH2Location(317, "Betwixt and Between Bond of Flame", locationCategory.POPUP,[locationType.TT]),]))
        g.add_node("TT-23",LocationNode([KH2Location(561, "Axel (Data) Magic Boost", locationCategory.POPUP,[locationType.TT, locationType.DataOrg]),]))

        return g

    def TWTNW(self):
        return self.create_list_from_nodes(self.twtnw_graph)

    def makeTWTNWGraph(self):
        g = Graph()

        g.add_node("TWTNW-1",LocationNode([KH2Location(374, "Fragment Crossing Mythril Stone", locationCategory.CHEST,[locationType.TWTNW]),
        ...
        g.add_node("TWTNW-2",LocationNode([KH2Location(69, "Roxas", locationCategory.HYBRIDBONUS,[locationType.TWTNW]),]))
        g.add_node("TWTNW-3",LocationNode([KH2Location(532, "Roxas Secret Ansem Report 8", locationCategory.POPUP,[locationType.TWTNW]),
                                            KH2Location(277, "Two Become One", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-4",LocationNode([KH2Location(391, "Memory's Skyscaper Mythril Crystal", locationCategory.CHEST,[locationType.TWTNW]),
        ...
        g.add_node("TWTNW-5",LocationNode([KH2Location(335, "The Brink of Despair Dark City Map", locationCategory.CHEST,[locationType.TWTNW]),
                                            KH2Location(500, "The Brink of Despair Orichalcum+", locationCategory.CHEST,[locationType.TWTNW]),]))
        g.add_node("TWTNW-6",LocationNode([KH2Location(378, "Nothing's Call Mythril Gem", locationCategory.CHEST,[locationType.TWTNW]),
                                            KH2Location(379, "Nothing's Call Orichalcum", locationCategory.CHEST,[locationType.TWTNW]),]))
        g.add_node("TWTNW-7",LocationNode([KH2Location(336, "Twilight's View Cosmic Belt", locationCategory.CHEST,[locationType.TWTNW]),]))
        g.add_node("TWTNW-8",LocationNode([KH2Location(23, "Xigbar Bonus", locationCategory.STATBONUS,[locationType.TWTNW]),]))
        g.add_node("TWTNW-9",LocationNode([KH2Location(527, "Xigbar Secret Ansem Report 3", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-10",LocationNode([KH2Location(380, "Naught's Skyway Mythril Gem", locationCategory.CHEST,[locationType.TWTNW]),
        ...
        g.add_node("TWTNW-11",LocationNode([KH2Location(278, "Oblivion", locationCategory.POPUP,[locationType.TWTNW]),
                                            KH2Location(496, "Castle That Never Was Map", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-12",LocationNode([KH2Location(24, "Luxord Bonus", locationCategory.HYBRIDBONUS,[locationType.TWTNW]),]))
        g.add_node("TWTNW-13",LocationNode([KH2Location(533, "Luxord Secret Ansem Report 9", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-14",LocationNode([KH2Location(25, "Saix Bonus", locationCategory.STATBONUS,[locationType.TWTNW]),]))
        g.add_node("TWTNW-15",LocationNode([KH2Location(536, "Saix Secret Ansem Report 12", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-16",LocationNode([KH2Location(535, "(Pre-Xemnas 1) Secret Ansem Report 11", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-17",LocationNode([KH2Location(385, "Ruin and Creation's Passage Mythril Stone", locationCategory.CHEST,[locationType.TWTNW]),
        ...
        g.add_node("TWTNW-18",LocationNode([KH2Location(26, "Xemnas 1 Bonus", locationCategory.DOUBLEBONUS, [locationType.TWTNW]),]))
        g.add_node("TWTNW-19",LocationNode([KH2Location(537, "Xemnas 1 Secret Ansem Report 13", locationCategory.POPUP,[locationType.TWTNW]),]))
        g.add_node("TWTNW-20",LocationNode([KH2Location(71, "Final Xemnas", locationCategory.STATBONUS,[locationType.TWTNW]),]))
        g.add_node("TWTNW-21",LocationNode([KH2Location(554, "Xemnas (Data) Power Boost", locationCategory.POPUP,[locationType.TWTNW, locationType.DataOrg]),]))
