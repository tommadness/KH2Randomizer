...
2. Extract the contents of the downloaded CustomImages.zip file. This will typically involve right-clicking on the zip file and selecting "Extract All" or using an unzip utility.

3. Make sure the extracted folder containing the custom images is in the same directory (folder) where the tracker program is located. In simpler terms, move the extracted folder next to the tracker program file.
...
4. To replace a default check image with your custom image, follow these steps:
 ...
 - Paste the copied custom image file into the same directory (folder) where the tracker program and the original image file are located. This will overwrite the default image file with your custom image.

Basically, you're giving your custom image the same exact filename as the default image so the tracker program knows to use your image instead.
