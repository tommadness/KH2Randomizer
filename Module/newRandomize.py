...
class Randomizer():
    def __init__(self, settings: RandomizerSettings):
        ...
        allLocations = Locations.getAllSoraLocations()
        ...
        invalidLocations = [loc for loc in allLocations if any(item in loc.LocationTypes for item in settings.disabledLocations)]
        validLocations =  [loc for loc in allLocations if not any(item in loc.LocationTypes for item in settings.disabledLocations)]
        ...
        self.assignKeybladeAbilities(settings, allAbilities, validLocations)
    ...
    def assignKeybladeAbilities(self, settings, allAbilities, validLocations):
        """Assign abilities to keyblades. """
        keybladeLocations =  [loc for loc in validLocations if locationType.WeaponSlot in loc.LocationTypes]
        ...
        for keyblade in keybladeLocations:
            ...
            validLocations.remove(keyblade)
    ...
    def assignItem(self,loc: KH2Location,item: KH2Item):
        ...
        if loc in self.assignedItems:
            assigned = [a for a in self.assignedItems if a==loc][0]
            ...
            self.assignedItems.append(ItemAssignment(loc,item))
