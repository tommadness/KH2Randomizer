...
class ProgressionDisplayWidget(QDialog):
    def __init__(self, progress_info: ProgressionPoints):
        ...
        for x in range(18):
            hint_threshold_label = QLabel(
                str(self.progress_info.get_point_threshold(x))
            )
        ...
        for w in world_names:
            ...
            for x in range(self.progress_info.get_num_cp_for_world(w)):
                box.addWidget(
                    QLabel(str(self.progress_info.get_cp_for_world(w, x))),
                    world_count,
                    x + 1,
                )
            world_count += 1
...
class ProgressionWidget(QWidget):
    def __init__(self, settings, setting_name):
        ...
        setting: ProgressionChainSelect = Class.seedSettings.settings_by_name[
            setting_name
        ]
        ...
        self.world_select.currentIndexChanged.connect(
            lambda index: self._change_visibility(self.world_options[index])
        )
        ...
        for w in self.world_options:
            ...
            for w_cp_index in range(setting.progression.get_num_cp_for_world(w)):
                world_cp_select_labels.append(
                    setting.progression.get_cp_label_for_world(w, w_cp_index)
                )
                spin_box = QSpinBox()
                ...
                spin_box.valueChanged.connect(
                    lambda value, world=w, world_cp=w_cp_index: self._change_setting(
                        world, world_cp, value
                    )
                )
            ...
            world_cp_select_box.currentIndexChanged.connect(
                lambda index, world=w: self._change_visibility_again(world, index)
            )
        ...
        self.threshold_select.currentIndexChanged.connect(
            lambda index: self._change_threshold_visibility(index)
        )
        ...
        for x in range(0, 18):
            ...
            spin_box.valueChanged.connect(
                lambda value, hint_index=x: self._change_threshold(hint_index, value)
            )
            # spin_box.lineEdit().setReadOnly(True)
        ...
        display_button.clicked.connect(
            lambda: ProgressionDisplayWidget(
                Class.seedSettings.settings_by_name[self.setting_name].progression
            ).exec()
        )
        self.full_layout.addWidget(display_button)

        export_button = QPushButton("Export CSV")
        export_button.clicked.connect(lambda: self.export_csv())
        self.full_layout.addWidget(export_button)

        import_button = QPushButton("Load CSV from Clipboard")
        import_button.clicked.connect(lambda: self.load_csv())
        self.full_layout.addWidget(import_button)
    ...
    def export_csv(self):
        ...
        csv_string = "Hint Thresholds"
        for x in range(0, 18):
            csv_string += ","
            csv_string += str(setting.progression.get_point_threshold(x))
        csv_string += "\n"
        world_names = setting.progression.get_world_options()
        for w in world_names:
            csv_string += w
            for x in range(setting.progression.get_num_cp_for_world(w)):
                csv_string += ","
    ...
    def load_csv(self):
        ...
        if len(split_data[0]) == 1:
            split_data = [
                list(filter(None, s.split("\t"))) for s in csv_string.splitlines()
            ]
        setting: ProgressionChainSelect = Class.seedSettings.settings_by_name[
            self.setting_name
        ]
        for row in split_data:
            row_name = row[0]
            data = row[1:]
            if row_name == "Hint Thresholds":
                for index, value in enumerate(data):
                    setting.progression.set_point_threshold(index, int(value))
            else:
                for world_cp, value in enumerate(data):
                    setting.progression.set_cp_for_world(row_name, world_cp, int(value))

        self._update_spinboxes()
        self.settings.set(self.setting_name, setting.progression.get_compressed())

        message = QMessageBox(text="Loaded point table from clipboard")
        message.setWindowTitle("KH2 Seed Generator")
        message.exec()
    ...
    def _update_spinboxes(self):
        ...
        for w in self.threshold_values:
            self.threshold_values[w].setValue(
                setting.progression.get_point_threshold(w)
            )
        for w in self.world_options:
            for w_cp_index in range(setting.progression.get_num_cp_for_world(w)):
                self.tier3_selects[w][w_cp_index].setValue(
                    setting.progression.get_cp_for_world(w, w_cp_index)
                )
